...
class TranslateNewProducts implements \Magento\Framework\Event\ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($product->isObjectNew()) {
            ...
            $targetStores = [];
            $sourceLocale = $this->helper->getStoreIdFromLocale($this->scopeConfig->getValue('globallink_translate_new_products/translate_new_products/tnp_source_locale', \Magento\Store\Model\ScopeInterface::SCOPE_STORE))[0];
            foreach ($stores as $store) {
                if ($store->getData("store_id") != $sourceLocale && $store->getData('locale') != '') {
                    $targetStores[] = $store->getId();
                }
            }
            $shortCode = $this->scopeConfig->getValue('globallink_translate_new_products/translate_new_products/tnp_project', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            $confirmationEmail = $this->scopeConfig->getValue('globallink_translate_new_products/translate_new_products/tnp_confirmation_email', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            $includeOptions = $this->scopeConfig->getValue('globallink_translate_new_products/translate_new_products/tnp_include_options', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            $numberOfDays = $this->scopeConfig->getValue('globallink_translate_new_products/translate_new_products/tnp_number_of_days', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);

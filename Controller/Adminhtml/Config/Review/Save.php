            ...
            if (!empty($fieldIds)) {
                try {
                    ...
                    if($this->logger->isDebugEnabled()) {
                        $this->logger->logAction(Data::PRODUCT_REVIEW_ID, Logger::CONFIG_ACTION_TYPE, $this->getRequest()->getParams());
                    }
                } catch (\Exception $e) {
                    if($this->logger->isErrorEnabled()) {
                        $this->messageManager->addErrorMessage($e->getMessage());
                        $this->logger->logAction(Data::PRODUCT_REVIEW_ID, Logger::CONFIG_ACTION_TYPE, $this->getRequest()->getParams(), Logger::CRITICAL, $e->getMessage());
                    }
                }
            }
        }
        $resultRedirect = $this->resultRedirectFactory->create();
        $resultRedirect->setPath('translations/config_review/field');
